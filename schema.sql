-- ESQUEMA DE BASE DE DATOS PARA THE BLUE DINO
...
-- 2. TABLA USUARIOS (Vinculada a auth.users de Supabase)
...
-- 6. TABLAS DE RELACIONES (JUNCTION TABLES)

-- Relación Padres - Pacientes (Muchos a Muchos: un padre puede tener 1-3 pacientes, un paciente 1-2 padres)
...
-- Relación Terapeutas - Pacientes (Muchos a Muchos: un terapeuta tiene N pacientes, un paciente N terapeutas)
...
-- 8. TABLA MENSAJES (Chat en tiempo real)
CREATE TABLE public.mensajes (
    ...
    tipo TEXT DEFAULT 'texto' NOT NULL -- 'texto', 'pictograma', 'voz'
);

-- 9. TABLA ALERTAS (Botón de pánico)
CREATE TABLE public.alertas (
    id_alerta UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    id_paciente UUID NOT NULL REFERENCES public.pacientes(id_paciente) ON DELETE CASCADE,
    ubicacion TEXT NOT NULL, -- Coordenadas en formato "lat,lng" o dirección
    fecha TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
);

-- 10. TABLA PICTOGRAMAS (Caché local/remota)
CREATE TABLE public.pictogramas (
    id_pictograma SERIAL PRIMARY KEY,
    categoria TEXT NOT NULL, -- 'personas', 'acciones', 'cosas', 'comida'
    texto TEXT NOT NULL,
    imagen TEXT NOT NULL -- URL o ruta del recurso de imagen
);
...
BEGIN
    -- Obtener el rol de la metadata de registro (por defecto 'paciente')
    ...
    -- Insertar en la tabla de perfil específico según corresponda
    IF v_rol = 'padre' THEN
...
-- ====================================================
-- CONFIGURACIÓN DE ROW LEVEL SECURITY (RLS) BÁSICA
-- ====================================================

-- Habilitar RLS en tablas críticas para seguridad
...
-- Políticas simplificadas de lectura/escritura para desarrollo (permitir acceso autenticado)
